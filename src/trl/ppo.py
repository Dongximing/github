...
def ppo_initialize(self, topic_link, prefix_link):
    topic_label = open(topic_link,'r').read().split('\n')
    prefix_label = open(prefix_link,'r').read().split('\n')
    prompt = []
    for i in range(140):
        topic, prefix = divmod(i, 20)
        text = topic_label[topic]+': '+ prefix_label[prefix]
        prompt.append([topic_label[topic], text])
    return prompt
...
class PPOTrainer:
    ...
    def batched_forward_pass(self, queries, responses):
        ...
        for i in range(int(bs/fbs)):
            ...
            input_ids = self.data_collator([torch.cat([r]) for q, r in zip(query_batch, response_batch)])["input_ids"]
            with torch.no_grad():
    ...
    def loss(self, old_logprobs, values, rewards, query, response, model_input):
        ...
        for t in reversed(range(gen_len)):
            ...
            print("nextvalues",len(nextvalues))
